...
onKeyCodes : List ( Int, msg ) -> Html.Attribute msg
onKeyCodes codeMsgPairs =
    let
        isExpectedCode currentCode =
            case List.filter (\p -> Tuple.first p == currentCode) codeMsgPairs of
                [] ->
                    Json.Decode.fail ("not " ++ String.fromInt currentCode)

                matchingList ->
                    case List.head matchingList of
                        Just ( _, msg_ ) ->
